...
enum AssignTargetNameMode<'a> {
    /// Assign target must be a basic variable.
    /// ```text
    /// name_only: true
    /// ```
    NameOnly,
    /// Assign target can be some complex name, be it by supporting namespaces,
    /// or by supporting tuples.
    /// ```text
    /// name_only: false
    /// (
...
{% macro insert_by_period(default) -%}
